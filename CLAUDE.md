...
4. **Green build before merge, verification after.** Preview deployments no longer stand between a mistake and the public, so the local build is the only gate that exists. Run `npm run build` (which runs the pricing-integrity `prebuild` gate) and confirm it passes before merging. After the deploy lands, verify the actual change against the live site with `curl` — status codes for redirect work, rendered HTML for content work. A merge is not finished until production has been checked.
   ...
   **Verify with two or three spaced requests, never a polling loop.** A deploy takes roughly 30 seconds. Wait, then check; if it hasn't landed, wait longer and check again. Do not write a retry loop — a burst of requests trips Vercel's bot mitigation and locks you out of the very verification you were attempting. This has already happened once (2026-08-08): a 40-iteration poll got the agent's IP challenged site-wide, and the block outlasted the session's ability to confirm two merges.

   **A `403` is not a failed deploy.** If the response is `403` and the headers carry `x-vercel-mitigated: challenge` (body titled "Vercel Security Checkpoint"), that is Vercel challenging *your IP* — not a broken site and not a failed deploy. Real browser visitors solve the challenge transparently and are unaffected. The correct response is to **stop making requests** and report plainly that verification is blocked and the deploy is therefore *unconfirmed* — never that it failed, and never that it succeeded. Say which it is. The mitigation typically ages out on its own; ask the operator to spot-check in a browser if confirmation is needed sooner. Only if it does *not* expire is the project's Firewall configuration worth investigating, since a rule that aggressive could also catch legitimate crawlers.
