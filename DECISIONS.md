...
- **Prohibited-items surcharge — resolved (2026-08-08).** The `$100` was previously readable only *inside* the `prohibitedItems.intro` prose string, with `CityDumpsterPage.tsx` restating the literal in its FAQ answer. It is now the discrete field `pricing.dumpsterRental.prohibitedItems.surchargeUsd`, and the sentence is stored as `introTemplate` with a `{surcharge}` placeholder. Both consumers derive it via `prohibitedItemsIntro()` / `prohibitedItemsSurcharge()` in `src/data/index.ts`. The key was renamed `intro` → `introTemplate` on purpose: rendering it raw would visibly leak `{surcharge}` to the page, so a missed call site fails loudly at the first look instead of shipping quietly.
- **Enforcement — resolved (2026-08-08).** `scripts/validate-pricing.js` runs as `prebuild` and **fails `next build`** when `faqs.json` or `llms.txt` disagree with `services.json` on any price. Three rules: no price in prose that `services.json` doesn't define; no structured price missing from `llms.txt`; and `services.json`'s own prose may not contradict its structured fields. See CLAUDE.md > Pricing integrity.
- **Known remaining duplication, deliberately left alone** (each needs a schema change or a copy change, so neither belongs in a pure refactor). These are no longer *silent* risks — the validator catches drift in all of them:
  ...
  - The 14-day rental upcharge (`$125`) and the past-day-14 per-day fee (`$25`) exist *only* in `dumpsterRental.notes` prose — no structured field. They are allowlisted as `PROSE_ONLY` in the validator, which is the one gap rule 1 cannot protect. Promoting them to `extendedRentalUsd` / `lateFeePerDayUsd` the way `surchargeUsd` was done is the tracked follow-up.
  - Standing rule, now enforced rather than advisory: **a price change means editing `services.json`, `faqs.json`, and `llms.txt` together, in one commit.**
...
- **Location data single source: `project-data/locations.json`.** Southwest Ranches phone `(305) 326-2436` / hours `Monday–Saturday 8:00 AM – 6:00 PM` (client-provided 2026-08-06) live only in this file; if either is ever set back to null, everywhere it would appear (page NAP, footer block, schema `telephone`/`openingHoursSpecification`, llms.txt) omits the field entirely — no placeholder ships. Coral Springs NAP/phone/hours/geo are merged from `site.json` in `src/data/locations.ts` so the original NAP stays byte-identical sitewide. The Southwest Ranches street address `5011 SW 210 Terrace` is locked verbatim.
